...
/**
 * ADAPTED: upstream derives `__workspace` by going up exactly one level from this file,
 * which is correct at `<repo>/scripts/` and wrong at `<repo>/tools/build/`. Replaced with
 * an upward search for `pnpm-workspace.yaml`, so moving this file later cannot silently
 * skew every resolved path.
 */
...
export function resolvePath(metaUrl) {
    ...
    const __root = process.env.PKG_DIR ? path.resolve(process.env.PKG_DIR) : path.resolve(path.dirname(fileURLToPath(metaUrl || import.meta.url)), '../');
    const __workspace = findWorkspaceRoot(path.dirname(fileURLToPath(import.meta.url)));
