...
/** The three things a declaration exposes, and the three places a caller writes them. */
const MEMBER_KINDS = new Set(['input', 'output', 'slot']);
...
const VALIDATORS = {
    ...
    get_module(payload, problems) {
        ...
        module.declarations.forEach((declaration, i) => {
            ...
            if (payload.view === 'summary') {
                ...
                // Slots are the exception, and the asymmetry is the point: an input can be
                // looked up later by name, but a slot name is unguessable and cheap, so the
                // summary NAMES them where it only counts the rest. Requiring the array here
                // stops that from silently degrading into a count.
                if (!Array.isArray(declaration?.slots) || declaration.slots.some((slot) => typeof slot !== 'string')) {
                    problems.push(`${at}: a summary must name its slots — an array of slot-name strings`);
                }
            } else {
                ...
                if (!Array.isArray(declaration?.slots)) problems.push(`${at}: slots must be an array`);
            }
        });
    ...
    search_api(payload, problems) {
        ...
        payload.matches.forEach((match, i) => {
            ...
            // Enumerated, not free text. `kind` is what tells a caller whether the match is
            // something they SET (`[label]="…"`) or something they FILL (`<ng-template #x>`),
            // and those are written in different places — a typo'd kind would read as a
            // fourth, non-existent way to use the library.
            if (match?.kind !== undefined && !MEMBER_KINDS.has(match.kind)) {
                problems.push(`matches[${i}]: kind must be one of ${[...MEMBER_KINDS].join(', ')}`);
            }
        });
    },
    ...
    corpus_info(payload, problems) {
        ...
        // The number alone invites a comparison against npm that means nothing: a release
        // stamps the version in CI and does not commit it back, so a checkout always trails.
        // Shipping the version without the caveat is how a correct corpus gets regenerated by
        // someone who read a lower number as staleness.
        if (!isString(payload?.libraryVersionNote)) {
            problems.push('libraryVersionNote must be a string — the version alone reads as staleness when it is not');
        }
